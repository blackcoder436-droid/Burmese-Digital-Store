    ...
    <div className="game-card p-6 space-y-5">
      ...
      <div className="flex flex-wrap gap-3 text-sm">
        ...
        <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border ${
          ...
          {isExpired ? tr('Expired', 'သက်တမ်းကုန်ပြီ') : `${daysLeft} ${tr('days left', 'ရက်ကျန်')}`}
        </span>
      </div>
      ...
      <div className="p-4 bg-dark-900 border border-dark-600/50 rounded-xl">
        <div className="flex items-center justify-between mb-2">
          <span className="text-xs text-gray-500 font-medium uppercase tracking-wider">
            {tr('VPN Key (Config Link)', 'VPN Key (Config Link)')}
          </span>
          <button
          ...
          >
            {copiedField === 'config' ? (
              <><Check className="w-3.5 h-3.5 text-emerald-400" /> {tr('Copied!', 'ကူးပြီး!')}</>
            ) : (
              <><Copy className="w-3.5 h-3.5" /> {tr('Copy Key', 'Key ကူးမည်')}</>
            )}
          </button>
        </div>
        ...
        >
          ...
          {showQr ? tr('Hide QR', 'QR ဖျောက်မည်') : tr('Show QR Code', 'QR Code ပြမည်')}
        </button>
        {showQr && qrDataUrl && (
          <div className="mt-3 flex justify-center">
            <div className="p-3 bg-dark-800 rounded-xl border border-purple-500/20 inline-block">
              ...
              <p className="text-[10px] text-gray-600 text-center mt-2">{tr('Scan with V2Box / V2RayTUN', 'V2Box / V2RayTUN ဖြင့် Scan လုပ်ပါ')}</p>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="p-4 bg-dark-900 border border-dark-600/50 rounded-xl">
        <div className="flex items-center justify-between mb-2">
          <span className="text-xs text-gray-500 font-medium uppercase tracking-wider">
            {tr('Subscription Link', 'Subscription Link')}
          </span>
          <div className="flex items-center gap-2">
            ...
            >
              {copiedField === 'sub' ? (
                ...
                <><Copy className="w-3.5 h-3.5" /> {tr('Copy', 'ကူးမည်')}</>
              )}
            </button>
            <a
              href={vpnKey.subLink}
      ...
      <div className="p-4 bg-dark-900 border border-dark-600/50 rounded-xl text-sm text-gray-400 space-y-2">
        <p className="font-semibold text-white text-sm mb-2">📱 {tr('How to use:', 'အသုံးပြုနည်း:')}</p>
        <ol className="list-decimal list-inside space-y-1.5">
          <li>{tr('Copy the VPN Key above', 'အထက်ပါ VPN Key ကို Copy လုပ်ပါ')}</li>
          <li>{tr('Open V2Box, V2RayTUN, or Shadowrocket app', 'V2Box, V2RayTUN, Shadowrocket app ဖွင့်ပါ')}</li>
          <li>{tr('Tap + → Import from clipboard', '+ ကိုနှိပ်ပါ → Clipboard မှ Import လုပ်ပါ')}</li>
          <li>{tr('Connect and enjoy!', 'Connect နှိပ်ပြီး အသုံးပြုပါ!')}</li>
        </ol>
      </div>

      {/* Expiry info */}
      <p className="text-xs text-gray-600">
        {tr('Expires:', 'သက်တမ်းကုန်:')} {expiryDate.toLocaleDateString()} {expiryDate.toLocaleTimeString()}
      </p>
    </div>
  );
}
