...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const dryRun = body.dryRun !== false; // default true -> dry-run unless explicitly set false
    ...
    log.info('Manual multi-server key repair completed', {
      recordId: id,
      updatedCount,
      failedCount,
      subLinksChanged,
    ...
    return NextResponse.json({
      success: true,
      message: dryRun ? `Dry-run completed: ${actions.length} action(s)` : `Repair completed: ${updatedCount} updated, ${failedCount} failed`,
      data: {
        actions,
        before: beforeReport,
        after: afterReport,
    ...
    log.error('Admin multi-server key repair error', {
      error: error instanceof Error ? error.message : String(error),
    });
    return NextResponse.json({ success: false, error: 'Failed to repair key' }, { status: 500 });
  }
}
