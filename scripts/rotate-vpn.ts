...
async function main() {
  ...
  if (!oldDroplet) {
      ...
      // DO requires some time to free up the quota after delete
      await notifyTg(`Waiting for 30s to allow DO to clear the Droplet limit...`);
      await sleep(30000); 
  }

  // Step 4: Create new VPS
  await notifyTg(`Creating New VPS (${config.dropletSize}) in ${dropletRegion}...`);
  const createPayload = {
      name: serverName,
      region: dropletRegion,
      size: config.dropletSize,
      image: config.dropletImage,
      ssh_keys: process.env.DO_SSH_KEY_ID ? [process.env.DO_SSH_KEY_ID] : [], // Use DO_SSH_KEY_ID if available
  };
  
  const createRes = await fetchDO('/droplets', { method: 'POST', body: createPayload });
  const newDropletId = createRes.droplet.id;
  
  await notifyTg(`VPS initialization started. Waiting for IP address assignment...`);
  
  let newIp = null;
  for (let i = 0; i < 15; i++) { // Try for ~2.5 mins
      await sleep(10000);
      const checkRes = await fetchDO(`/droplets/${newDropletId}`);
      newIp = checkRes.droplet.networks?.v4?.find(n => n.type === 'public')?.ip_address;
      if (newIp) break;
  }
  
  if (!newIp) {
      await notifyTg(`❌ Failed to retrieve IP address for new droplet. Rotation aborted.`);
      process.exit(1);
  }

  await notifyTg(`New VPS created successfully! IP: ${newIp}`);

  // Step 5: Update Cloudflare DNS
  await notifyTg(`Updating Cloudflare DNS for ${targetDomain}...`);
  try {
      // 5.1 Get Zone ID (if not provided in Env)
      let zoneId = process.env.CF_ZONE_ID;
      if (!zoneId) {
          const zonesRes = await fetchCF(`/zones?name=burmesedigital.store`);
          zoneId = zonesRes.result[0]?.id;
      }
      
      // 5.2 Get specific DNS Record ID
      const recordsRes = await fetchCF(`/zones/${zoneId}/dns_records?name=${targetDomain}&type=A`);
      const record = recordsRes.result[0];

      if (record) {
          // 5.3 Update existing record
          await fetchCF(`/zones/${zoneId}/dns_records/${record.id}`, {
              method: 'PUT',
              body: {
                  type: 'A',
                  name: targetDomain,
                  content: newIp,
                  proxied: false, // Must be false for 3X-UI raw connection initially, or follow your setup
                  ttl: 120 // very short TTL to propagate changes quickly
              }
          });
      } else {
          // 5.4 Create record if not found
          await fetchCF(`/zones/${zoneId}/dns_records`, {
              method: 'POST',
              body: { type: 'A', name: targetDomain, content: newIp, proxied: false, ttl: 120 }
          });
      }
      await notifyTg(`Cloudflare DNS Updated successfully.`);
  } catch (err) {
      await notifyTg(`❌ Cloudflare DNS Update Failed: ${err.message}`);
      // Usually we might want to exit, but let's continue with installation or maybe manual DNS fix later
  }

  // NOTE: SSH scripts for Installation will be added next.
  console.log("Phase 2 done. VPS Created and DNS Updated.");
  process.exit(0);
}
